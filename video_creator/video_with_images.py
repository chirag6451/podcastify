...
########################################################################
def create_video_with_images(audio_path, config, resolution, request_dict, show_speakers=False):
    """
    ...
    try:
        ...
        temp_output = os.path.join(tempfile.gettempdir(), f"video_with_images_{random.randint(1000, 9999)}.mp4")
        output_path = os.path.join(config.get('output_dir'), config.get('main_video_filename'))
        image_clips = []
        ...
        base_video.write_videofile(output_path, fps=24, audio_codec='aac')
